...
// // Define types for place details and photos
...
function MapWithPlaceAutocomplete() {
  ...
  const handleMapClick = async (event: any) => {
    ...
    //     const request = {
    //       location: latLng,
    //       radius: '50', // Set a small radius for nearby places
    //       types: ['establishment'], // Specify types if necessary
    //     };

    //     // Instead of using nearbySearch, we'll use the getDetails method
    //     service.nearbySearch(request, (results, status) => {
    //       if (status === window.google.maps.places.PlacesServiceStatus.OK && results) {
    //         const place = results[0]; // Get the first place result
    //         if (place && place.place_id) {
    //           setPlaceId(place.place_id); // Set the place ID
    //           // console.log('Place ID from map click:', place.place_id);
    //           fetchPlaceDetails(place.place_id); // Fetch details for the clicked place
    //         } else {
    //           console.log('No places found near the clicked location.');
    //         }
    //       } else {
